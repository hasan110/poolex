<template>
    <div>
        <div class="container-fluid" style="max-width: 800px;">
            <h3 class="text-white p-3">ویرایش بلاگ</h3>
            <div class="row">
                ...
                <div class="col-md-6 col-sm-12">
                    <div class="form-group text-white">
                        ...
                        <small v-if="errors.new_thumbnail" class="invalid-feedback">
                            {{errors.new_thumbnail[0]}}
                        </small>
                        <div v-if="thumbnail_preview" class="p-1">
                            <img style="height: 100px;width: auto;object-fit: contain;" :src="thumbnail_preview">
                        </div>
                        <div v-else-if="blog.thumbnail" class="p-1">
                            <img style="height: 100px;width: auto;object-fit: contain;" :src="ImageUrl + blog.thumbnail">
                        </div>
                    </div>
                </div>
                <div class="col-md-6 col-sm-12">
                    <div class="form-group text-white">
                        ...
                        <div v-else-if="blog.banner" class="p-1">
                            <img style="height: 100px;width: auto;object-fit: contain;" :src="ImageUrl + blog.banner">
                        </div>
                    </div>
                </div>
            ...
            <button class="btn btn-success my-3" @click="editBlog()">ثبت</button>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods:{
        fileSelected(type){
            if(type === 'banner'){
                this.blog.new_banner = this.$refs.banner.files[0];
                this.banner_preview = URL.createObjectURL(this.blog.new_banner);
            }
            else if(type === 'thumbnail'){
                this.blog.new_thumbnail = this.$refs.thumbnail.files[0];
                this.thumbnail_preview = URL.createObjectURL(this.blog.new_thumbnail);
            }
        },
        getBlog(){
            this.SPIN_LOADING(1)
            this.$axios.get(`blogs/blog/${this.blog_id}`)
                .then(res => {
        ...
        editBlog(){
            ...
            this.$axios.post(`blogs/edit` , form,
                {
                    headers: {
